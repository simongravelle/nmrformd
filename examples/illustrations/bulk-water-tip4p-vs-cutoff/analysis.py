...
path = "/data/nmrformd-data/bulk-water-tip4p-vs-cutoff/raw_data/"
run = True
R1_intra = 1e-5
R1_inter = 1e-5
while run:
    for cutoff in np.arange(7, 14, 1):
        # Read current status
        name_intra = "N4000_intra_cutoff" + str(cutoff)
        name_inter = "N4000_inter_cutoff" + str(cutoff)
        n_intra = measure_N(name_intra)
        n_inter = measure_N(name_inter)
        # Import MDA universe
        datapath = path + "N4000_300K_co" + str(cutoff) + "/"
        ...
        print("cutoff =", cutoff, " --", n_intra, n_hydrogen, 
